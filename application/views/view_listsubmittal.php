 ...
 <div class="wrapper row-offcanvas row-offcanvas-left">
					...
					<div class="row">
						<div class="col-lg-12">
						  ...
						  <h3 align="center">Search - Document Register</h3>
							<form class="form-horizontal adv-srch" onSubmit="javascript: return false;">
								<div class="row">
								  <?php 
								  ...
								  for($i=0; $i<$loopCounter; $i++){ ?>
                                    <div class="form-group col-lg-4 col-md-6 col-sm-12">
                                        ...
                                        <select class="form-control filterQueryInput chosen-select" multiple="multiple" name="<?php echo $searchFields[$i]['fieldname']?>" id="<?php echo $searchFields[$i]['fieldname']?>">
								...
								<div class="row">
									...
									<div class="col-md-4">
										<select id="column_control" name="column_control[]" class="chosen-select form-control" multiple="multiple">
											<?php foreach ($searchColumns as $key => $field) { ?>
                                         	<option value="<?php echo $field['fieldname']; ?>" <?php if($key < 5){?>selected<?php } ?>><?php echo $field['label']; ?></option>
                                         	<?php } ?>
										</select>
									</div>
		...
		<script type="text/javascript">
			var pageNumber = 1,
				rowPerPage,
				solrSearchUrl = "/solrsearch/select";
			$(document).ready(function(){
				$(".chosen-select").chosen();
				doSearch();	// Fetch all records on page load

				/*$('.multiselect-ui').multiselect({
					includeSelectAllOption: true,
					buttonWidth: '100%'
				});*/
				$('#sort_by, #column_control, #rows').change(function(){
					$('form.adv-srch').submit();
				});
				$('form.adv-srch').submit(doSearch);

				function doSearch(e){
					e && e.preventDefault();
					var searchQueryData = prepareFilterQuery($('form.adv-srch').find(".filterQueryInput").serialize());
					...
					var columnsArr = $("#column_control").val(),
						...
						requestUrl = solrSearchUrl+"?q=*:*&start="+start+"&rows="+$("#rows").val()+"&wt=json&fl="+columnsArr.join(',')+searchQueryData.fq;
				...
				function prepareFilterQuery(data){
					...
					for(var i=0; i<dataArr.length; i++){
						...
						if(srchData[1] != '' && srchData[0]){
							if((/\s/g.test(srchData[1].replace(/\+/, " ")))){
								srchData[1].split("+").join("* AND *")
								srchData[1] = srchData[1].split("+").join("* AND *");
							}
